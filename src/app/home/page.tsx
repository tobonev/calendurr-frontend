...
export default function Home() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await fetch('https://calendurr-backend.onrender.com/api/v1/auth/me', {
          credentials: 'include',
        });
        ...
        const schedulesResponse = await fetch('https://calendurr-backend.onrender.com/api/v1/plans', {
          credentials: 'include',
        });
  ...
  const handleDelete = async (scheduleId: string) => {
    if (window.confirm('Are you sure you want to delete this schedule?')) {
      try {
        const response = await fetch(`https://calendurr-backend.onrender.com/api/v1/plans/${scheduleId}`, {
          method: 'DELETE',
          credentials: 'include',
        });
